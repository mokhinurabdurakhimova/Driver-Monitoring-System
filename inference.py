...
def distance(p1, p2):
    ...
    Hisoblash ko'z otasidahi masonani hisonlash icjin kerak ndi, uning hilllati turli xil bo'lin u;;atni
    kuzatush ketak
...
def Attendence(frame_before_run, ear_main, mar_main, puc_main, moe_main):
    with open("Attendence.csv", "r+") as file:
        data = file.readline()
        file.writelines(f"\n{frame_before_run},{ear_main},{mar_main},{puc_main},{moe_main}")
...
def run_face_mp(image):
    ...
    results_img = face_mesh.process(image)

    image.flags.writeable = True
    image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)

    if results_img.multi_face_landmarks:
        landmarks_positions = []
        # assume that only face is present in the image
        for _, data_point in enumerate(results_img.multi_face_landmarks[0].landmark):
        ...
        for face_landmarks in results_img.multi_face_landmarks:
...
def infer(ears_norm, mars_norm, pucs_norm, moes_norm):
    ...
    while cap.isOpened():
            ...
            for faceLms in results.multi_face_landmarks:
                for lm in faceLms.landmark:
                    ...
                    if z <= -20 or z > 18:
                        print("Checked...")
                        print(z)
                        draw_spec = mp_drawing.DrawingSpec(color=(0, 0, 255))
                    else:
                        draw_spec = mp_drawing.DrawingSpec(color=(255, 255, 255))
        ...
        frame_before_run += 1
        if frame_before_run >= 1 and len(input_data) == 20:
            frame_before_run = 0
            label = get_classification(input_data)
        # here you can give apppend to csv file:
        Attendence(frame_before_run, ear_main, mar_main, puc_main, moe_main)
